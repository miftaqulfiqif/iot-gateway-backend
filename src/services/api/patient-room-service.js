...
export const getRoomsService = async () => {
  try {
    ...
    const formattedRooms = rooms.map((room) => {
      const roomStatus =
        room.patient_room.length === room.capacity ? "full" : "available";
...
export const getRoomsWithGatewaysService = async () => {
  try {
    const rooms = await prismaClient.room.findMany({
      where: {
        iot_gateway: {
...
export const getRoomsForCreateGatewayService = async (query) => {
  try {
    const whereClause = {};
    ...
    const rooms = await prismaClient.room.findMany({
      ...
      orderBy: { name: "asc" }, // opsional: agar hasil rapi
    });

    const formatted = rooms.map((room) => ({
      id: room.id,
      label: `${room.name} ${room.number} - ${room.type}`,
    }));
