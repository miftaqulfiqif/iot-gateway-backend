...
} from "../../services/api/devices-model/doppler-service.js";

const create = async (req, res, next) => {
  try {
    const result = await createService(req.user, req.body);
    res.status(200).json({ message: "Doppler created", data: result });
  } catch (error) {
    next(error);
  }
};
...
const getAll = async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "Success getting dopplers",
...
const getByPatientId = async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "Success getting doppler by patient id",
...
const getByDeviceId = async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "Success getting doppler by device id",
      device_id: deviceId,
      current_page: page,
      total_items: result.total,
      total_pages: Math.ceil(result.total / limit),
...
const getByUserId = async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      message: `Success getting doppler by user id : ${userId}`,
      current_page: page,
      total_items: result.total,
      total_pages: Math.ceil(result.total / limit),
      data: result.data,
