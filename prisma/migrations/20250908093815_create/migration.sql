...
-- CreateTable
CREATE TABLE `recent_measurements` (
    `id` VARCHAR(191) NOT NULL,
    `patient_handler_id` VARCHAR(191) NOT NULL,
    `parameter` ENUM('weight', 'blood_pressure', 'temperature', 'sp02') NOT NULL,
    `value` VARCHAR(100) NOT NULL,
    `room` VARCHAR(255) NULL,
    `recorded_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    UNIQUE INDEX `recent_measurements_patient_handler_id_key`(`patient_handler_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- AddForeignKey
ALTER TABLE `recent_measurements` ADD CONSTRAINT `recent_measurements_patient_handler_id_fkey` FOREIGN KEY (`patient_handler_id`) REFERENCES `patient_handlers`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
