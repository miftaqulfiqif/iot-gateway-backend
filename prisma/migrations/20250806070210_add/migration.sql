...
-- AlterTable
ALTER TABLE `devices_connected` ADD COLUMN `count_used` INTEGER NULL;

-- AlterTable
ALTER TABLE `measurement_activities` MODIFY `description` LONGTEXT NULL;
...
-- AddForeignKey
ALTER TABLE `patient_handlers` ADD CONSTRAINT `patient_handlers_device_id_fkey` FOREIGN KEY (`device_id`) REFERENCES `devices_connected`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
